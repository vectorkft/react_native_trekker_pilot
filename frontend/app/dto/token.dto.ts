...
export const TokenDTOInput = z.object({
    refreshToken: z.string().refine(token => jwtRegex.test(token), "Érvénytelen JWT token"),
});
export const TokenDTOOutput = z.object({
    newAccessToken: z.string().refine(token => jwtRegex.test(token), "Érvénytelen JWT token"),
});
